...
│
├── src/feedback_triage/  # Installable package (src/ layout)
...
│   ├── scripts_cli.py           # Entry point wrappers for fta-* CLI commands
...
| ------------- | -------------------- | ----------------------------------------------------------------- |
| `fta`         | `main:main`          | Primary CLI — parses args via `cli.py`, dispatches to `engine.py` |
| `fta-version` | `main:print_version` | Print package + Python version                                    |
| `fta-doctor`  | `main:doctor`        | Diagnose environment (venv, tools, config files)                  |
| `fta-start`   | `main:start`         | Start the application (alias for `fta run`)                       |
