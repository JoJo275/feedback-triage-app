...
| **UV Lock Refresh**       | [uv-lock-refresh.yml](../.github/workflows/uv-lock-refresh.yml)             | push (path-filtered), weekly, manual                    | `Refresh uv.lock`             | Runs `uv lock --upgrade` and opens a PR when `uv.lock` changes                                |
