...
Covers the template's `bootstrap.py` / `customize.py` flow used to spin
up new projects from `simple-python-boilerplate`. The fork into
`feedback-triage-app` is a one-time event and does not need automated
onboarding. The relevant scripts are being removed; this ADR is archived.
