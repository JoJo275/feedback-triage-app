...
**Deprecated** (archived 2026-04-27). Previously: Accepted — Revised (v2).

Describes the `tools/dev_tools/env_dashboard/` FastAPI app that surfaced
template diagnostic data. The dashboard and its collectors are being
removed during the fork into `feedback-triage-app`; the only FastAPI app
in this project is the production app itself.
