...
Applies to the template's broad `scripts/` surface (doctor, customize,
bootstrap, env collectors, etc.). The `feedback-triage-app` keeps only a
small, purpose-specific set of scripts (`scripts/seed.py`); per-script
smoke tests in CI are not warranted. Archived.
