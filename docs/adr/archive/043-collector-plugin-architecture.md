...
Describes the plugin contract for `scripts/_env_collectors/` consumed by
the env-inspection dashboard (see ADR 041). Both the collectors and the
dashboard are being removed during the fork; the architecture they
defined is no longer in scope for `feedback-triage-app`.
