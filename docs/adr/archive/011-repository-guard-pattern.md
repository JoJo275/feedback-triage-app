...
**Deprecated** (archived 2026-04-27).

This ADR was inherited from the `simple-python-boilerplate` template. The
guard pattern protected forks of the template from running optional
workflows before they were configured. The `feedback-triage-app` is a
single concrete project, not a template, so optional workflows either
apply unconditionally or are removed. The guard pattern is not used here.
