...
Describes the `repo_doctor` / profile-based diagnostic system that lived
in the template. The `feedback-triage-app` does not ship doctor scripts
or profiles; verification is handled by `task check` and CI. Archived
because the implementation is being removed during the template fork.
