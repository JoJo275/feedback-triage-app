...
<!-- Generated: 2026-05-20 22:59:27 UTC -->
...
!!! info "Generation details"
    ...
    | **Timestamp** | 2026-05-20 22:59:27 UTC |
...
| `task check` | Run backend + frontend quality gates (incl. CSS and web builds) |
...
| Command | Description |
| ------- | ----------- |
| `task email:replay` | Re-send a failed email_log row: task email:replay -- <log-uuid> |
...
| Command | Description |
| ------- | ----------- |
| `task seed:demo:large` | Reset demo-owner workspace with large demo volume (1248 signals) |
| `task seed:demo:small` | Reset demo-owner workspace with small demo volume (25 signals) |
| `task seed:workspace` | Seed a v2 workspace with rich demo data: task seed:workspace -- --slug demo-owner --reset |
...
| `task test:db:reset` | Drop/recreate isolated pytest DB and migrate to head |
...
| Command | Description |
| ------- | ----------- |
| `task web:audit` | Run npm audit at high severity threshold |
| `task web:build` | Build frontend assets and Vite manifest into static/app |
| `task web:contracts` | Export backend OpenAPI and generate typed frontend contracts |
| `task web:contracts:check` | Verify generated frontend contracts are in sync with OpenAPI |
| `task web:install` | Install web dependencies with npm ci |
| `task web:lint` | Run frontend lint checks |
| `task web:test` | Run frontend unit tests |
| `task web:typecheck` | Run frontend type checks |
...
**File:** [`scripts/email_replay.py`](../../scripts/email_replay.py)

??? note "Full `--help` output"

    ```text
    usage: email_replay [-h] [--dry-run] [--smoke] [-q] [-v] [--version] [log_id]

    Re-send a failed transactional email. Loads the email_log row by id, asserts it is in a replayable terminal state, and re-issues the send via the same EmailClient that the API uses (subject to RESEND_DRY_RUN).

    positional arguments:
      log_id         UUID of the email_log row to replay (omit with --smoke).

    options:
      -h, --help     show this help message and exit
      --dry-run      Load + validate the row but do not call the client.
      --smoke        Self-check (no DB / no network); exit 0 on success.
      -q, --quiet    Suppress informational output (repeatable).
      -v, --verbose  More log detail (repeatable).
      --version      show program's version number and exit

    Examples:
      task email:replay -- 11111111-2222-3333-4444-555555555555
      python scripts/email_replay.py 11111111-2222-3333-4444-555555555555 --dry-run
...
**File:** [`scripts/seed_workspace.py`](../../scripts/seed_workspace.py)

??? note "Full `--help` output"

    ```text
    usage: seed_workspace.py [-h] [--slug SLUG] [--count COUNT] [--reset]
                             [--force] [--seed SEED] [--smoke]
                             [--create-if-missing] [--owner-email OWNER_EMAIL]
                             [--owner-password OWNER_PASSWORD] [--version]

    Populate a workspace with realistic demo data.

    options:
      -h, --help            show this help message and exit
      --slug SLUG           Target workspace slug (default: demo-owner).
      --count COUNT         Number of feedback items to insert (default: 150).
      --reset               Delete the workspace's existing tags, submitters, and
                            feedback first.
      --force               Insert even if the workspace already has feedback.
      --seed SEED           RNG seed for deterministic output (default: 42).
      --smoke               Run a self-check (no DB access) and exit.
      --create-if-missing   Create the workspace (and owner user) if no row
                            matches --slug.
      --owner-email OWNER_EMAIL
                            Email for the owner user when --create-if-missing
                            creates it.
      --owner-password OWNER_PASSWORD
                            Initial password for the owner user when --create-if-
                            missing creates it.
      --version             show program's version number and exit
...
| Run backend + frontend quality gates (incl. CSS and web builds) | `task check` |
...
| Re-send a failed email_log row: task email:replay -- <log-uuid> | `task email:replay` |
...
| Reset demo-owner workspace with large demo volume (1248 signals) | `task seed:demo:large` |
| Reset demo-owner workspace with small demo volume (25 signals) | `task seed:demo:small` |
| Seed a v2 workspace with rich demo data: task seed:workspace -- --slug demo-owner --reset | `task seed:workspace` |
...
| Drop/recreate isolated pytest DB and migrate to head | `task test:db:reset` |
...
| Run npm audit at high severity threshold | `task web:audit` |
| Build frontend assets and Vite manifest into static/app | `task web:build` |
| Export backend OpenAPI and generate typed frontend contracts | `task web:contracts` |
| Verify generated frontend contracts are in sync with OpenAPI | `task web:contracts:check` |
| Install web dependencies with npm ci | `task web:install` |
| Run frontend lint checks | `task web:lint` |
| Run frontend unit tests | `task web:test` |
| Run frontend type checks | `task web:typecheck` |
